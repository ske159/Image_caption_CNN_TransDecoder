...
@torch.no_grad()
def val_coco(model, val_image_folder, device, train_dataset, result_i, transform, batch_size):
    ...
    data = pd.read_csv('./coco/val2014_caption.txt')
    if result_i > 315:
        result_i = 0
    else:
        pass

    for i in range(result_i * batch_size, (batch_size + result_i * batch_size) * 5, 5):
        ...
        dict_for_receive["caption"] = bioclean(" ".join(model.caption_image(img.to(device), train_dataset.vocab)[1:-1]))
